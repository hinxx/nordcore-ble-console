...
`tools/treadmill_app.py` — a bigger sibling of `controller_ui.py`: same Control tab, plus a History tab backed by a local SQLite log (`tools/treadmill_history.db`, gitignored). Logging only happens while this app is open and connected — no separate background service. Every TELEMETRY sample is logged with a step delta computed the same reset-aware way `fw/controller/main/uart_rx.c` accumulates steps across speed changes, just one level up, and distinguishes the two different reasons the firmware's reported step count can decrease: at a real stop (reported speed is 0), it's a genuine reset -- a fresh local segment starts. Otherwise, since that count is still an 8-bit value wrapping at 256, a decrease while still moving is a wraparound, not a reset -- the full wrap gets credited (`(256 - last) + new`) instead of being mistaken for a reset and losing a step at every rollover. Daily/weekly totals are then a plain `SUM()` over the log, so they survive power cycles and app restarts. The History tab also shows an estimated distance (speed integrated over time) and a bar chart with three views -- today by hour (zero-filled across all 24, so you can see when walking actually happened during the day, not just a single flat daily total), daily (last 14 days), and weekly (last 12 weeks) -- and tracks two step counts side by side, since the hardware's own count can be unreliable at some speeds: the real one, and a speed-derived estimate (`distance covered / step length`, where step length comes from a configurable height via the standard pedometer formula `step_length_cm ~= height_cm * 0.415`). A radiobutton picks which one the chart plots; the summary text always shows both. This is a rough estimate, not a replacement for the real sensor -- real step length isn't perfectly speed-independent the way the formula assumes, so if it doesn't match reality at your walking pace, the height field is the one knob to tune (even past your literal height) to calibrate it. The Control tab also has two things `controller_ui.py` doesn't: a configurable play speed (PLAY ramps straight to whatever's set there, e.g. 2.0 km/h, instead of always landing on the firmware's fixed 0.8 km/h floor and needing manual Speed Up clicks afterward -- SET_SPEED sent from a full stop engages the ramp exactly like PLAY does, so this needs no firmware change), and an auto-stop safety watchdog: PLAY starts the belt rolling whether or not anyone steps on it, so if the reported step count hasn't changed for a configurable timeout (default 10s), it sends STOP on its own -- a red "Stopping in Ns" countdown appears once 2s have passed with no step, giving a clear, visible warning before it actually fires. Both fields fall back to their defaults on blank/invalid input rather than silently misbehaving, and both are persisted to the same SQLite database (a `settings` key/value table) as soon as they're edited, so they're already loaded next time the app starts instead of resetting to defaults. `pip install bleak matplotlib && python3 tools/treadmill_app.py`.
