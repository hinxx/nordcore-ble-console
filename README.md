...
Closing the window minimizes it to a system tray icon (`pystray`, forced to the GTK `StatusIcon`/XEmbed backend -- the AppIndicator backend's dynamic icon updates are unreliable on Linux and it has no click-to-activate) instead of quitting -- the BLE connection and history logging keep running in the background. The dot's color reflects state (gray disconnected, blue connected-idle, green running) and hovering shows current speed plus this session's step count, or the last completed session's count while idle. **Left-click the tray icon to restore the window**; right-click for the menu ("Show Treadmill", and "Quit" -- the only way to actually exit).
