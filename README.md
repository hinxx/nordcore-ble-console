...
Closing the window minimizes it to a system tray icon (`pystray`, via AppIndicator/StatusNotifierItem on Linux) instead of quitting -- the BLE connection and history logging keep running in the background. The dot's color reflects state (gray disconnected, blue connected-idle, green running) and hovering shows current speed plus this session's step count, or the last completed session's count while idle. The tray menu's "Show Treadmill" (also its default click action) restores the window; "Quit" is the only way to actually exit.
