...
| `fw/controller/` | 1.0.0 | Active control, not passive sniffing — commands the baseboard directly (play/stop/set-speed) over a new TX line, replacing the stock console's control role, while still reading `BASE->CON` telemetry (speed, steps) the same way the passive sniffers do. The first firmware here that transmits onto a treadmill UART line. See `fw/controller/DESIGN.md` for the full hardware and protocol plan this implements. |

Each firmware's version lives in two places kept in sync by hand: `fw/<name>/version.txt` (which ESP-IDF embeds into the compiled binary's app description — check it later with `esptool.py image_info` or over OTA) and an `FW_VERSION` macro in that firmware's `main.c` (which is what actually gets printed in the serial boot banner). Confirmed by building all four and checking the configure-step output: `App "treadmill_frame_sniffer" version: 1.0.0`, `App "treadmill_byte_sniffer" version: 1.0.0`, `App "treadmill_ble_sniffer" version: 1.0.0`, `App "treadmill_controller" version: 1.0.0`, each independent of the others and of the repo's own git state. Bump a firmware's version in both places together when its behavior changes.

`./tools/idf.sh` defaults to `frame-sniffer`; pass `-f <name>` (or `--fw <name>`) before the `idf.py` arguments to target a different one, e.g. `./tools/idf.sh -f byte-sniffer build`, `./tools/idf.sh -f ble-sniffer build`, or `./tools/idf.sh -f controller build`.
...
`tools/treadmill_app.py` — a bigger sibling of `controller_ui.py`, three tabs:

- **Control**: Play / Stop / Speed Up / Speed Down + live telemetry -- "Steps: N (est: M)" shows the hardware's own reported count next to the same speed-derived estimate History uses, live, so a mismatch is visible immediately rather than only after checking History; both reset together at a real stop (see History below for the same estimate over time) -- plus an auto-stop safety watchdog -- PLAY starts the belt rolling whether or not anyone steps on it, so if the reported step count hasn't changed for the configured timeout, it sends STOP on its own. A red "Stopping in Ns" countdown appears once 2s have passed with no step, giving a clear warning before it actually fires.
- **History**: a local SQLite log (`tools/treadmill_history.db`, gitignored) of every TELEMETRY sample received while this app is connected -- logging only happens while it's open, no separate background service. Each sample's step delta is computed the same reset-aware way `fw/controller/main/uart_rx.c` accumulates steps across speed changes, one level up, and distinguishes the two different reasons the firmware's reported count can decrease: a real stop (reported speed is 0) is a genuine reset; a decrease while still moving is that count's 8-bit wraparound at 256, not a reset, and gets the full wrap credited (`(256 - last) + new`) instead of losing a step at every rollover. Daily/weekly totals are then a plain `SUM()` over the log, so history survives power cycles and app restarts. Shows an estimated distance (speed integrated over time) and a bar chart with three views -- today by hour (zero-filled across all 24, so separate walking sessions in a day show up as distinct bars, not one flat total), daily (last 14 days), weekly (last 12 weeks) -- and tracks two step counts side by side, since the hardware's own can be unreliable at some speeds: the real one, and a speed-derived estimate (`distance covered / step length`, from a configurable height via the standard pedometer formula `step_length_cm ~= height_cm * 0.415`). This is a rough estimate, not a replacement for the real sensor -- real step length isn't perfectly speed-independent the way the formula assumes, so if it doesn't match reality at your walking pace, height is the one knob to tune (even past your literal height) to calibrate it.
- **Settings**: all persisted preferences in one panel -- play speed (PLAY ramps straight to whatever's set here instead of always landing on the firmware's fixed 0.8 km/h floor; SET_SPEED sent from a full stop engages the ramp exactly like PLAY does, so this needed no firmware change), auto-stop timeout, and height. Each falls back to a sane default on blank/invalid input rather than silently misbehaving, and each is persisted to the same SQLite database (a `settings` key/value table) as soon as it's edited -- no separate Save action, and already loaded next time the app starts.

Closing the window minimizes it to a system tray icon (`pystray`, AppIndicator backend) instead of quitting -- the BLE connection and history logging keep running in the background. The dot's color and hover tooltip are meant to reflect live state (gray disconnected, blue connected-idle, green running, plus current speed and this session's step count), but note: AppIndicator-backed icon repaints are known-unreliable on Linux, and this host's alternative (the GTK `StatusIcon`/XEmbed backend, forced via `PYSTRAY_BACKEND=gtk`) was tried and reverted after it crashed xfce4-panel's `systray` plugin outright -- see the comment above the `pystray` import in `tools/treadmill_app.py` before picking that fight again. **Click the tray icon to open its menu** -- "Show Treadmill" restores the window, "Quit" is the only way to actually exit; AppIndicator icons have no separate click-to-activate.
