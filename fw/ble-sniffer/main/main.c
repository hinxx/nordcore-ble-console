...
#define LINE_A_BLE_DIR          BLE_SNIFFER_DIR_CON_TO_BASE
...
#define LINE_B_BLE_DIR          BLE_SNIFFER_DIR_BASE_TO_CON
 ...
 *
 * Same parser as fw/frame-sniffer -- only the delivery of a completed, valid
 * frame differs: instead of a printf line, it goes out over BLE (see
 * ble_sniffer_notify_frame below). MANGLED/ERROR stay on USB serial since
 * they're rare; see README.md for why the continuous per-frame stream
 * specifically moved to BLE.
 */
...
/* Non-static: shared with ble_gatt.c so BLE lifecycle log lines never
 * interleave with the sniffer tasks' own MANGLED/ERROR console lines. */
...
/*
 * Print one line for anything that didn't parse as a complete, checksum-
 * valid frame ("MANGLED <reason> ..."), so a mangled capture is never
 * mistaken for a real message downstream. `bytes` is whatever was actually
 * captured -- a partial or stray run.
 */
static void print_frame_mangled(const char *name, int64_t ts, const char *reason,
                                 const uint8_t *bytes, size_t len)
{
    ...
    xSemaphoreTake(g_console_mutex, portMAX_DELAY);
    printf("%012" PRId64 " %s MANGLED %s %s\n", ts, name, reason, hex);
    fflush(stdout);
    xSemaphoreGive(g_console_mutex);
}
...
static void frame_feed_byte(frame_parser_t *p, const sniff_line_t *line, uint8_t byte, int64_t ts)
{
    ...
    if (end_byte != FRAME_END_BYTE) {
        ...
        ble_sniffer_notify_frame(line->ble_dir, p->buf, p->count);
    }
    p->count = 0;
}
...
void app_main(void)
{
    ...
    printf("BLE: advertising as \"TreadmillSniffer\"; valid frames go out over\n");
    printf("     the RX_LOG notify characteristic as <direction><len><frame bytes>\n");
    printf("     once a client subscribes -- see README.md.\n");
    printf("USB serial now only carries this banner, BLE lifecycle lines, and:\n");
    printf("  MANGLED: <timestamp_us> <direction> MANGLED <reason> <hex bytes...>\n");
    printf("  ERROR:   <timestamp_us> <direction> ERROR <reason>\n\n");
    fflush(stdout);
